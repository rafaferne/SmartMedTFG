...
export default function UploadStressCSV({ onImported }) {
  ...
  const submit = async () => {
    if (!file) {
      setMsg({ type: "warning", text: "Selecciona un archivo .csv con datos de estrés" });
      return;
    }
    setLoading(true);
    setMsg(null);
    try {
      ...
      fd.append("file", file);
      const res = await apiFetch("/import/stress/csv", { method: "POST", body: fd });
      const json = await res.json();
      if (!res.ok || json.ok === false) throw new Error(json.error || "Error importando CSV");

      const bulk = await apiFetch("/ai/score/stress/from_csv/bulk_llm", { method: "POST" });
      const bj = await bulk.json();
      if (!bulk.ok || bj.ok === false) throw new Error(bj.error || "Error valorando el estrés con IA");
  ...
  return (
    <Card sx={{ mt: 3 }}>
      <CardHeader
        title="Importar datos de estrés"
