...
import os, requests, json, re, hashlib, time, base64
...
def _simulate_prompt(metric: str, profile: dict, recent_points: list, horizon_min: int):
    ...
    return (
        ...
        f"Histórico reciente (1=más antiguo): {recent_str or 'sin datos'}\n"
        ...
        "    ... (1 a 3 elementos)\n"
        "  ],\n"
        '  "forecast": [ {"minute": 1, "value": 3}, {"minute": 2, "value": 3}, ... ]\n'
        "}\n"
...
def metrics_ingest():
    ...
    mongo.db.measurements.insert_one({
        "sub": sub, "type": mtype, "value": value, "ts": ts,
...
def _call_llm(prompt: str, model: str = None, max_tokens: int = 2000, max_retries: int = 1):
    ...
    - Si finishReason == MAX_TOKENS, intenta parsear JSON parcial y, si no, error claro.
    Env:
    ...
    model    = (model or os.getenv("LLM_MODEL") or "gemini-2.5-flash").strip()
    ...
    instruction = (
        "Eres un asistente de bienestar. NO des consejo médico. "
        "Devuelve SOLO un JSON válido exacto: "
        '{"score": <1..5>, "rationale": "<máx 10 palabras>"}'
    )
    ...
    while True:
        try:
            ...
            if resp.status_code in (429, 503) and attempt < max_retries:
                ...
                time.sleep(min(wait_s, 10.0)); attempt += 1; continue
            ...
            if pf.get("blockReason"):
                raise RuntimeError(f"gemini_blocked:{pf.get('blockReason')}")
            ...
            if finish == "MAX_TOKENS":
                ...
                raise RuntimeError("Respuesta Gemini inesperada (finishReason: MAX_TOKENS)")

            if finish not in (None, "STOP"):
                # p.ej. SAFETY / OTHER
                raise RuntimeError(f"gemini_blocked:{finish}")
...
# -------------------- PROMPTS DE PUNTUACIÓN --------------------

def _sleep_prompt(profile: dict, payload: dict) -> str:
    sex = profile.get("sex"); birthdate = profile.get("birthdate")
    height = profile.get("height_cm"); weight = profile.get("weight_kg")
    notes = (payload.get("notes") or "").strip()

    hours = payload.get("hours"); awakenings = payload.get("awakenings")
    deep = payload.get("deep_minutes"); rem = payload.get("rem_minutes")
    quality = payload.get("quality")

    return f"""
Evalúa el SUEÑO (1..5) y devuelve JSON con 'score' y 'rationale' (breve).
No des consejo médico; considera perfil y medidas.

PERFIL: sex={sex}, height_cm={height}, weight_kg={weight}, birthdate={birthdate}
EPISODIO: hours={hours}, awakenings={awakenings}, deep_minutes={deep}, rem_minutes={rem}, quality_subjective={quality}, notes='{notes}'
"""
...
def _activity_prompt(profile: dict, payload: dict) -> str:
    ...
    minutes = payload.get("minutes"); intensity = payload.get("intensity")
    steps = payload.get("steps"); hr_zone_minutes = payload.get("hr_zone_minutes")

    return f"""
Evalúa la ACTIVIDAD (1..5) y devuelve JSON con 'score' y 'rationale' (breve).
No des consejo médico.

PERFIL: sex={sex}, height_cm={height}, weight_kg={weight}, birthdate={birthdate}
ACTIVIDAD: minutes={minutes}, intensity={intensity}, steps={steps}, hr_zone_minutes={hr_zone_minutes}, notes='{notes}'
"""
...
def _store_scored_point(sub: str, mtype: str, score: int):
    ...
    mongo.db.measurements.update_one(
        ...
        {"$set": {"sub": sub, "type": mtype, "ts": now, "value": score, "ingested_at": datetime.utcnow()}},
        upsert=True
    )
    return now
